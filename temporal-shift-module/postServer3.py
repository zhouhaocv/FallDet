...
weight_file = 'SOTA_model/TSM_falldet_RGB_resnet50_shift8_blockres_avg_segment8_e50_Room_w_shad/ckpt.best.pth.tar'
num_class = 5
test_segments = 8
img_feature_dim = 256
test_crops = 1
...
twice_sample = False
softmax = False
classnames = ['fall', 'stand', 'squat', 'lying', 'sit']
...
def YV12toRGB(byteArray, width, height):
    e = width * height
    Y = byteArray[0:e]
    Y = np.reshape(Y, (height, width))

    V = byteArray[e:int((5*e)/4)]
    V = np.repeat(V, 2, 0)
    V = np.reshape(V, (int(height / 2), width))
    V = np.repeat(V, 2, 0)

    U = byteArray[int((5*e)/4):]
    U = np.repeat(U, 2, 0)
    U = np.reshape(U, (int(height / 2), width))
    U = np.repeat(U, 2, 0)
...
def AlertAction():
    ...
    for i in range(len(image_list)):
        ...
        if image_type =='YV12':
            img = YV12toRGB(img_array, 1280, 720)
        else:

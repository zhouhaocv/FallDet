...
parser.add_argument('--dataset', type=str, default='falldet')

# may contain splits
parser.add_argument('--weights', type=str, default='/raid/file2/code2/tsm/checkpoint/TSM_falldet_RGB_resnet50_shift8_blockres_avg_segment8_e50/ckpt.best.pth.tar')
...
for i, data_label_pairs in enumerate(zip(*data_iter_list)):
    with torch.no_grad():
        ...
        prec1, prec2 = accuracy(torch.from_numpy(ensembled_predict), this_label, topk=(1, 2))
        top1.update(prec1.item(), this_label.numel())
        top5.update(prec2.item(), this_label.numel())
        if i % 20 == 0:
...
for ii in range(0,len(video_pred),50):
    print('results',ii,':\n',video_pred[ii:(ii+50)])
print(cf)
print(cls_acc)

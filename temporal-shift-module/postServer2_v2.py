...
def Sendframe():
    ...
    image_PIL = []
    image = base64.b64decode(image_list[0])
    ...
    if len(imagedatalist) >= frame_number:
        imagedatalist.pop(0)
    image1 = transform(image_PIL)
    imagedatalist.append(image1)

    # decode_crop_time = time.time() - proc_start_time1
...
def AlertAction():
    ...
    if not request.get_data():
        ...
        return jsonify(return_data)
    data = json.loads(request.get_data())
    if not len(imagedatalist) == data['frame_number']:
        print("Error: frame num receive: " + str(len(imagedatalist) ))
        return_data = {
            'code': -1
        }
        return jsonify(return_data)

    # image_list = np.concatenate(imagedatalist,axis=0)
    # image_all = torch.from_numpy(image_list)
    image_all = torch.cat(imagedatalist, axis=0)
    del imagedatalist[0:4]
    cnt_time1 = time.time()
    print('process time: {:.3f} sec/video'.format(float(cnt_time1- proc_start_time)))

    net.eval()
    try:
        results = eval_video(image_all, net, test_segments, modality)
        cnt_time = time.time()
        print('{:.3f} sec/video'.format(float(cnt_time- cnt_time1)))
